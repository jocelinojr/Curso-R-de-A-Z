...
meu.grafico <- function(dataset, rows=1:10, titulo=""){
  Dados <- dataset[rows, , drop=FALSE]
  matplot(t(Dados), type = "b", pch=15:18, col = c(1:4, 6))
  legend("bottomleft", inset = 0.01, legend = Players[rows], pch=15:18, col = c(1:4, 6), horiz = FALSE, title = titulo)
...
# Visualiza os dados com a nossa função 
meu.grafico(Salary/Points, rows = 1:10, titulo = "Salário/Pontos")
meu.grafico(Games, c(1:4), titulo = "Jogos")
meu.grafico(Salary/FieldGoals)
meu.grafico(Points, rows = 1:4, titulo = "Pontos")
